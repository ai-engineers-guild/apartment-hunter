...
class IngestPipeline:
    ...
    async def run_profile(self, profile: SearchProfile) -> list[Apartment]:
        ...
        for source_name in profile.sources:
            adapter_cls = _ADAPTERS.get(source_name)
            if not adapter_cls:
            ...
            try:
                apartments = await adapter.fetch_listings(
        ...
        if self.analyzer and new_apartments:
            log.info("Analyzing %d new apartments...", len(new_apartments))
